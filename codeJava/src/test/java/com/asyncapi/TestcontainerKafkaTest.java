...
public class TestcontainerKafkaTest {
    ...
    @Test
    public void pubProdutoAlteradoProducerTestcontainers() {
        ProdutoPayload payload = new ProdutoPayload();
        Integer key = 1;
        Integer wrongKey = key + 1;

        consumeMessages(PUB_PRODUTO_ALTERADO_SUBSCRIBE_TOPIC);

        publisherService.pubProdutoAlterado(key, payload);

        ConsumerRecord<Integer, Object> consumedMessage = consumeMessage(PUB_PRODUTO_ALTERADO_SUBSCRIBE_TOPIC);
    ...
    @Test
    public void subProdutoAlteradoConsumerTestcontainers() throws Exception {
        Integer key = 1;
        ProdutoPayload payload = new ProdutoPayload();
